...
function handler(argv) {
  ...
  var cmd = new ImportCommand([argv.pathToRepo], argv, argv._cwd);
...
var ImportCommand = function (_Command) {
  ...
  _createClass(ImportCommand, [{
    ...
    value: function createPatchForCommit(sha) {
      ...
      return patch.replace(/^([-+]{3} [ab])/gm, replacement).replace(/^(diff --git a)/gm, replacement).replace(/^(diff --git \S+ b)/gm, replacement).replace(/^(rename (from|to)) /gm, `$1 ${formattedTarget}/`);
    }
  }, {
    key: "execute",
